...
\begin{itemize}
    ...
    \item \textbf{Cautious to variance (Written by DeepSeek)}
    ...
    \item \textbf{Keep the variance seed and the EWMA updates in the same unit (Written by DeepSeek)}

    The very first variance I computed used \emph{raw price changes}
    \(S_t - S_{t-1}\), which gives a dollar variance (e.g., \(\sim 50\,\text{USDT}^2\)).
    But the EWMA inside the backtest loop updates the variance with
    \emph{log‑returns} \(\ln(S_t / S_{t-1})\), which are dimensionless and tiny.
    Mixing the two meant that on the first few snapshots the “dollar variance” was
    scaled twice by \(S_t^2\), giving absurdly wide spreads—the bot quoted spreads
    of thousands of dollars for the first few minutes until the EWMA’s memory faded.

    The fix is to let \texttt{get\_return\_var} compute the variance of log‑returns
    as well:
    \texttt{np.log(df[ref] / df[ref].shift(1)).var()}.
    Now the seed is dimensionless, just like the EWMA updates, and the later
    conversion \(\texttt{var} \times S_t^2\) correctly gives a dollar variance from
